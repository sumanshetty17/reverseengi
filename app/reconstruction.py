...
from app.sanitize import sanitize_content, env_example_lines
from app.scaffold_backend import write_scaffold_backend
...
def write_server_js(dest: Path, endpoints: List[Dict], original_url: str) -> None:
    ...
    server = f"""/**
 * YOUR clone server — visual reconstruction of: {original_url}
 * Clone visitors (sign-up / forms) are stored in data/db.json for YOU.
 * Original site customers are never copied.
...
if (!fs.existsSync(DB)) {{
  fs.writeFileSync(DB, JSON.stringify({{ users: [], sessions: {{}}, submissions: [], collections: {{}} }}, null, 2));
}}
...
app.post("/api/auth/register", (req, res) => {{
  ...
  const db = load();
  if (db.users.some((u) => u.email === email)) return res.status(409).json({{ ok: false, error: "already registered on this clone" }});
  const user = {{ id: crypto.randomUUID(), email, passwordHash: hash(password), createdAt: new Date().toISOString() }};
  db.users.push(user);
  const sid = crypto.randomBytes(16).toString("hex");
  db.sessions[sid] = user.id;
  save(db);
  res.setHeader("set-cookie", "carbon_sid=" + sid + "; Path=/; HttpOnly; SameSite=Lax");
...
app.post("/api/auth/login", (req, res) => {{
  ...
  if (!user || !check(password, user.passwordHash)) return res.status(401).json({{ ok: false, error: "invalid login on this clone" }});
  const sid = crypto.randomBytes(16).toString("hex");
  db.sessions[sid] = user.id;
  save(db);
  res.setHeader("set-cookie", "carbon_sid=" + sid + "; Path=/; HttpOnly; SameSite=Lax");
...
app.post("/api/data/:name", (req, res) => {{
  const name = String(req.params.name || "items").replace(/[^a-z0-9_]/gi, "") || "items";
  const db = load();
  if (name === "submissions") {{
    db.submissions.push({{ id: crypto.randomUUID(), payload: req.body, createdAt: new Date().toISOString() }});
  }} else {{
    db.collections[name] = db.collections[name] || [];
    db.collections[name].push({{ id: crypto.randomUUID(), payload: req.body, createdAt: new Date().toISOString() }});
  }}
  save(db);
  res.json({{ ok: true }});
}});
...
app.get("/api/data/:name", (req, res) => {{
  const name = String(req.params.name || "items").replace(/[^a-z0-9_]/gi, "") || "items";
  const db = load();
  if (name === "submissions") return res.json(db.submissions);
...
for (const p of STUB_PATHS) {{
  app.all(p, (req, res) => {{
    res.json({{
      ...
      stub: true,
      path: p,
      message: "Placeholder for a client-visible API from the source. Wire YOUR backend here.",
      method: req.method,
    }});
  }});
}}

app.get("/api/health", (_req, res) => res.json({{ status: "ok", reconstructed: true, cloneOwnedData: true }}));
...
app.listen(PORT, "0.0.0.0", () => {{
  console.log("Clone site (YOUR data in data/db.json) on port " + PORT);
}});
"""
...
def write_env_example(dest: Path, endpoints: List[Dict], report: Dict[str, Any] | None = None) -> None:
    techs = []
    if report:
        techs = [x.get("name", "") for x in (report.get("technologies") or []) if x.get("name")]
    body = env_example_lines(techs)
    extra = []
    for e in (endpoints or [])[:20]:
        u = e.get("url") or ""
        if u:
            extra.append(f"# Stubbed endpoint from public client: {u}")
    if extra:
        body += "\n".join(extra) + "\n"
    (dest / ".env.example").write_text(body, encoding="utf-8")
...
Public frontend material only. **Private API keys are stripped.** Publishable keys are replaced with placeholders. Add **your own** keys in `.env` (see `.env.example`). Backend behavior is stubbed until you connect your own API.
"""
...
def build_reconstruction(
    ...
    if downloads_dir and Path(downloads_dir).exists():
        for item in Path(downloads_dir).rglob("*"):
            ...
            try:
                # Sanitize JS/CSS/HTML/JSON so private keys never ship
                if item.suffix.lower() in {".js", ".mjs", ".css", ".html", ".htm", ".json", ".txt", ".map", ".svg"}:
                    ...
                    if flat.resolve() != target.resolve():
                        ...
                        flat.write_text(clean, encoding="utf-8")
                else:
    ...
    for i, (url, pdata) in enumerate(page_items):
        ...
        (recon_root / fname).write_text(sanitize_content(rewritten), encoding="utf-8")
    ...
    for r in routes:
        ...
        if fpath.exists():
            ...
            fpath.write_text(sanitize_content(html), encoding="utf-8")
...
""",
    ...
    write_env_example(recon_root, endpoints, report)
    ...
    # 1:1 scaffold clone: synthetic backend + mock data (no private keys)
    if scaffold_clone:
        try:
            write_scaffold_backend(recon_root, report, original_url)
        except Exception as _sc_err:
            (recon_root / "backend" / "SCAFFOLD_ERROR.txt").parent.mkdir(parents=True, exist_ok=True)
            (recon_root / "backend" / "SCAFFOLD_ERROR.txt").write_text(str(_sc_err), encoding="utf-8")
