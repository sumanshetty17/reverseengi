"""
Download and organize all accessible assets (CSS, JS, images, fonts, etc.).
Maintain resource map: original URL → local path → referencing pages.
"""

from __future__ import annotations

import hashlib
import mimetypes
import re
from pathlib import Path
from typing import Any, Dict, List, Optional, Set
from urllib.parse import urlparse, unquote

import httpx

from app.crawler import USER_AGENT, normalize_url, is_asset_url
from app.privacy import browser_headers, resolve_proxy


def safe_filename(url: str, max_len: int = 120) -> str:
    """Deterministic local filename from URL."""
    parsed = urlparse(url)
    path = unquote(parsed.path)
    name = path.rstrip("/").split("/")[-1] or "index"
    # strip query noise
    name = re.sub(r"[^\w.\-]", "_", name)
    if len(name) > max_len:
        stem, ext = (name.rsplit(".", 1) + [""])[:2]
        name = stem[: max_len - len(ext) - 9] + "_" + hashlib.md5(url.encode()).hexdigest()[:8]
        if ext:
            name = f"{name}.{ext}"
    if not name or name == "_":
        name = hashlib.md5(url.encode()).hexdigest()[:12]
    return name


def guess_type(url: str, content_type: str = "") -> str:
    path = urlparse(url).path.lower()
    ct = (content_type or "").lower()
    if any(path.endswith(e) for e in (".css",)) or "text/css" in ct:
        return "css"
    if any(path.endswith(e) for e in (".js", ".mjs")) or "javascript" in ct:
        return "javascript"
    if any(path.endswith(e) for e in (".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg", ".ico", ".avif")):
        return "image"
    if any(path.endswith(e) for e in (".woff", ".woff2", ".ttf", ".eot", ".otf")):
        return "font"
    if path.endswith(".json") or "application/json" in ct:
        return "json"
    if path.endswith(".map"):
        return "sourcemap"
    if path.endswith((".html", ".htm")):
        return "html"
    return "other"


def local_subdir(asset_type: str) -> str:
    # Everything under public/ so static hosting serves assets at predictable paths
    return {
        "css": "public/css",
        "javascript": "public/js",
        "image": "public/images",
        "font": "public/fonts",
        "json": "public/data",
        "sourcemap": "public/js",
        "html": "public/pages",
        "other": "public/assets",
    }.get(asset_type, "public/assets")



def candidate_urls(url: str, page_origin: str = "") -> list[str]:
    out: list[str] = []
    def add(u: str) -> None:
        if u and u not in out:
            out.append(u)
    add(url)
    try:
        from urllib.parse import urlparse, parse_qs, unquote, urljoin
        u = urlparse(url)
        if "/_next/image" in u.path or "/_vercel/image" in u.path or "url=" in (u.query or ""):
            qs = parse_qs(u.query)
            inner = (qs.get("url") or [None])[0]
            if inner:
                inner = unquote(inner)
                add(inner)
                if inner.startswith("/") and page_origin:
                    add(urljoin(page_origin, inner))
                elif not inner.startswith("http") and page_origin:
                    add(urljoin(page_origin, inner))
        if u.query:
            add(f"{u.scheme}://{u.netloc}{u.path}")
    except Exception:
        pass
    return out


class AssetCollector:
    def __init__(self, base_dir: Path, max_assets: int = 150, timeout: float = 15.0, proxy: str | None = None, cookie_header: str | None = None):
        self.base_dir = Path(base_dir)
        self.max_assets = max_assets
        self.timeout = timeout
        self.proxy = resolve_proxy(proxy)
        self.cookie_header = cookie_header
        self.manifest: List[Dict[str, Any]] = []
        self.url_to_local: Dict[str, str] = {}
        self.downloaded: Set[str] = set()
        self.failed: List[Dict[str, str]] = []

    async def download_one(
        self, client: httpx.AsyncClient, url: str, referenced_by: Optional[List[str]] = None
    ) -> Optional[Dict[str, Any]]:
        url = normalize_url(url)
        if url in self.downloaded or len(self.manifest) >= self.max_assets:
            return self.url_to_local.get(url) and {
                "original_url": url,
                "local_path": self.url_to_local[url],
                "status": "already_collected",
            }

        page_origin = ""
        try:
            from urllib.parse import urlparse
            if referenced_by:
                page_origin = f"{urlparse(referenced_by[0]).scheme}://{urlparse(referenced_by[0]).netloc}"
        except Exception:
            pass
        tries = candidate_urls(url, page_origin)
        last_err = "unknown"
        content = None
        content_type = ""
        ok_url = url
        for candidate in tries:
            for attempt in range(3):
                try:
                    headers = {}
                    if page_origin:
                        headers["Referer"] = page_origin + "/"
                        headers["Origin"] = page_origin
                    headers["Accept"] = "image/avif,image/webp,image/apng,image/*,font/*,*/*;q=0.8"
                    resp = await client.get(
                        candidate, follow_redirects=True, timeout=self.timeout + attempt * 4, headers=headers
                    )
                    if resp.status_code < 400 and resp.content:
                        content = resp.content
                        content_type = resp.headers.get("content-type", "")
                        ok_url = candidate
                        break
                    last_err = f"HTTP {resp.status_code}"
                    if resp.status_code == 404:
                        break
                except Exception as e:
                    last_err = str(e)
                import asyncio
                await asyncio.sleep(0.25 * (attempt + 1))
            if content is not None:
                break

        if content is None:
            self.failed.append({"url": url, "error": last_err})
            entry = {
                "original_url": url,
                "local_path": None,
                "type": guess_type(url),
                "referenced_by": referenced_by or [],
                "status": "failed",
                "error": last_err,
            }
            self.manifest.append(entry)
            return entry

        asset_type = guess_type(ok_url, content_type)
        subdir = local_subdir(asset_type)
        filename = safe_filename(ok_url)
        dest_dir = self.base_dir / subdir
        dest_dir.mkdir(parents=True, exist_ok=True)
        dest = dest_dir / filename
        counter = 1
        while dest.exists():
            stem = dest.stem
            dest = dest_dir / f"{stem}_{counter}{dest.suffix}"
            counter += 1

        dest.write_bytes(content)
        local_rel = str(dest.relative_to(self.base_dir)).replace("\\", "/")
        sha = hashlib.sha256(content).hexdigest()

        entry = {
            "original_url": url,
            "local_path": local_rel,
            "type": asset_type,
            "size": len(content),
            "content_type": content_type,
            "referenced_by": referenced_by or [],
            "status": "collected",
            "sha256": sha,
            "fetched_as": ok_url,
        }
        self.manifest.append(entry)
        self.url_to_local[url] = local_rel
        self.downloaded.add(url)
        return entry


    async def collect_many(
        self, urls: List[str], referenced_by: Optional[Dict[str, List[str]]] = None
    ) -> List[Dict[str, Any]]:
        referenced_by = referenced_by or {}
        unique = list(dict.fromkeys(urls))[: self.max_assets]
        headers = browser_headers(USER_AGENT)
        if self.cookie_header:
            ch = self.cookie_header.strip()
            if ch.lower().startswith("cookie:"):
                ch = ch.split(":", 1)[1].strip()
            headers["Cookie"] = ch
        results = []
        kw = dict(headers=headers, verify=True, follow_redirects=True)
        if self.proxy:
            kw["proxy"] = self.proxy
        async with httpx.AsyncClient(**kw) as client:
            for u in unique:
                refs = referenced_by.get(u, [])
                r = await self.download_one(client, u, refs)
                if r:
                    results.append(r)
        return results

    def get_manifest(self) -> List[Dict[str, Any]]:
        return self.manifest
