...
def parse_cookies_input(raw: str, default_url: str = "") -> list:
    """
    Accept cookies from the user after they logged in manually.
    Formats:
      - JSON array of Playwright cookie objects
      - browser "name=value; name2=value2" header string
...
async def apply_cookies_to_context(context, raw_cookies: str, base_url: str) -> int:
    """
    Accept cookies from the user after they logged in manually.
    Formats:
...
async def render_page(
    ...
    try:
        if own_browser:
            ...
            # proxy from login dict optional key or env handled by caller via login["_proxy"]
            _proxy = (login or {}).get("_proxy") if login else None
            if _proxy:
                ctx_kwargs["proxy"] = {"server": _proxy}
            context = await browser.new_context(**ctx_kwargs)
            page = await context.new_page()
            if login and login.get("session_cookies"):
                try:
                    cookies = parse_cookies_input(
                        login.get("session_cookies") or "",
                        default_url=login.get("login_url") or url,
                    )
                    if cookies:
                        await context.add_cookies(cookies)
                        result["login"] = {"ok": True, "method": "session_cookies", "count": len(cookies)}
                except Exception as e:
                    result["login"] = {"ok": False, "method": "session_cookies", "error": str(e)}
        else:
            p = None
            browser = None
        ...
        # Unlock scroll gates / cookie walls so the rest of the page exists
        gate_info = {"gates": [], "scroll_locked_initially": False, "overflow_before": "", "overflow_after": ""}
        try:
            gate_info = await page.evaluate("""async () => {
              const gateRe = /(scroll\\s*to\\s*explore|scroll\\s*down|scroll\\s*to\\s*discover|explore|enter\\s*(the\\s*)?(site|experience)|discover|view\\s*more|skip\\s*(intro|animation)|accept(\\s*(all|cookies))?|agree|got\\s*it|allow(\\s*(all|cookies))?|continue|start\\s*experience)/i;
              function vis(el){
                const r = el.getBoundingClientRect();
                const cs = getComputedStyle(el);
                return r.width>8 && r.height>8 && cs.visibility!=='hidden' && cs.display!=='none' && cs.opacity!=='0';
              }
              function selector(el){
                if (el.id) return '#'+CSS.escape(el.id);
                const t = el.tagName.toLowerCase();
                const cls = (el.className && typeof el.className==='string')
                  ? '.'+el.className.trim().split(/\\s+/).slice(0,3).map(c=>CSS.escape(c)).join('.')
                  : '';
                return t+cls;
              }
              const overflowBefore = getComputedStyle(document.documentElement).overflow + ' / ' + getComputedStyle(document.body||document.documentElement).overflow;
              const scrollLockedInitially = /hidden|clip/.test(overflowBefore)
                || document.documentElement.style.overflow==='hidden'
                || (document.body && document.body.style.overflow==='hidden');
              const gates = [];
              for (const el of document.querySelectorAll('a,button,[role="button"],div,span,p')) {
                const text = (el.innerText||'').trim().replace(/\\s+/g,' ');
                if (!text || text.length>42 || !gateRe.test(text) || !vis(el)) continue;
                const r = el.getBoundingClientRect();
                gates.push({ text, tag: el.tagName, selector: selector(el), clicked: false });
              }
              const extra = [
                '#onetrust-accept-btn-handler','.cc-accept','.cookie-accept','button[id*="accept"]',
                '[class*="scroll-indicator"]','[class*="scroll-hint"]','[class*="scrollTo"]','[data-scroll-to]'
              ];
              ...
              for (const g of gates) {
                try {
                  ...
                  if (el) { el.click(); g.clicked = true; }
                } catch(e) {}
              }
              document.documentElement.style.overflow = 'auto';
              if (document.body) document.body.style.overflow = 'auto';
              document.documentElement.classList.remove('no-scroll','is-locked','lenis-stopped','overflow-hidden');
              if (document.body) document.body.classList.remove('no-scroll','is-locked','overflow-hidden');
              if (window.lenis && typeof window.lenis.start==='function') try { window.lenis.start(); } catch(e){}
              await new Promise(r => setTimeout(r, 400));
              const overflowAfter = getComputedStyle(document.documentElement).overflow + ' / ' + getComputedStyle(document.body||document.documentElement).overflow;
              return { gates, scroll_locked_initially: scrollLockedInitially, overflow_before: overflowBefore, overflow_after: overflowAfter };
            }""")
        except Exception as e:
            gate_info = {"gates": [], "scroll_locked_initially": False, "error": str(e)}

        result["gate_info"] = gate_info

        # Deep scroll to trigger lazy assets after unlock
        try:
            for _ in range(8):
                await page.evaluate("window.scrollBy(0, Math.max(400, document.body.scrollHeight / 6))")
...
async def render_with_session(
    ...
    async with async_playwright() as p:
        ...
        login_info = None
        # Human-in-the-loop: cookies from user after they solved CAPTCHA/login themselves
        if login and login.get("session_cookies"):
            try:
                cookies = parse_cookies_input(
                    login.get("session_cookies") or "",
                    default_url=login.get("login_url") or (urls[0] if urls else ""),
                )
                if cookies:
                    await context.add_cookies(cookies)
                    login_info = {"ok": True, "method": "session_cookies", "count": len(cookies)}
            except Exception as e:
                login_info = {"ok": False, "method": "session_cookies", "error": str(e)}

        if login and login.get("username") and login.get("password") and not (login_info and login_info.get("ok")):
