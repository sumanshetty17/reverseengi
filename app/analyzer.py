...
# Jobs: memory + disk so Render restarts / multi-request still find status
_JOBS: Dict[str, Dict[str, Any]] = {}
JOBS_DIR = REPORTS_DIR / "_jobs"
...
def persist_job(job_id: str) -> None:
    ...
    # Avoid huge payloads blowing status files; keep summary fields
    slim = {k: v for k, v in job.items() if k != "report"}
    if job.get("report") is not None:
        slim["report"] = job["report"]
    _job_path(job_id).write_text(json.dumps(slim, default=str), encoding="utf-8")
...
def list_jobs() -> List[Dict[str, Any]]:
    ...
    for path in JOBS_DIR.glob("*.json"):
        ...
        if jid not in _JOBS:
            try:
                _JOBS[jid] = json.loads(path.read_text(encoding="utf-8"))
            except Exception:
                pass
    return [
        {
...
async def analyze(
    ...
    persist_job(job_id)
    # Free-tier safety: never exceed these even if client asks higher
    max_pages = max(1, min(int(max_pages or 12), 20))
    max_assets = max(1, min(int(max_assets or 80), 120))
    ...
    try:
        ...
        persist_job(job_id)
        cookie_header = None
        if session_cookies and not session_cookies.strip().startswith("["):
            cookie_header = session_cookies.strip()
            if cookie_header.lower().startswith("cookie:"):
                cookie_header = cookie_header.split(":", 1)[1].strip()

        try:
            crawler = Crawler(
                url, max_pages=max_pages, depth=depth, proxy=proxy_url, cookie_header=cookie_header
            )
        except TypeError:
            try:
        ...
        if browser_mode or login or session_cookies:
            try:
                _JOBS[job_id]["message"] = (
                    "Using your session cookies + browser render…"
                    if session_cookies else
                    ("Logging in (authorized) + browser render…" if login else "Rendering with browser engine…")
                )
                ...
                if login or session_cookies:
                    session_results = await render_with_session(
                        primary, login=login, proxy=proxy_url, session_cookies=session_cookies
                    )
                else:
                    session_results = {}
                    for pu in primary:
                ...
                browser_findings["error"] = str(browser_err)
                _JOBS[job_id]["message"] = f"Browser step skipped: {browser_err}. Continuing with HTTP crawl…"
                persist_job(job_id)
        ...
        collector = AssetCollector(job_downloads, max_assets=max_assets, proxy=proxy_url, cookie_header=cookie_header)
        ...
        _html_for_infer = "\n".join(
            (p.get("rendered_html") or p.get("html") or "") for p in (pages or {}).values()
        )
        backend_inference = infer_backend_from_frontend(
            _html_for_infer, endpoints, technologies, purpose, pages
        )
