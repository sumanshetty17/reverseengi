...
def build_report(
    ...
    missing: List[str],
    backend_inference: Optional[Dict] = None,
) -> Dict[str, Any]:
    pages_summary = []
    ...
    report = {
        ...
        "backend_inference": backend_inference or {},
...
<body>
  ...
  <h2>Inferred backend, data & business logic (from frontend only)</h2>
  <p style="color:#64748b;font-size:0.9em;">{(report.get('backend_inference') or {}).get('disclaimer') or ''}</p>
  <h3>Likely backend style</h3>
  <ul>
    {''.join(f"<li>{x}</li>" for x in ((report.get('backend_inference') or {}).get('likely_backend_style') or [])) or '<li>Unknown</li>'}
  </ul>
  <h3>Inferred data domains</h3>
  <ul>
    {''.join(f"<li>{x}</li>" for x in ((report.get('backend_inference') or {}).get('inferred_data_domains') or [])) or '<li>Unknown</li>'}
  </ul>
  <h3>Inferred entities</h3>
  <ul>
    {''.join(f"<li><code>{x}</code></li>" for x in ((report.get('backend_inference') or {}).get('inferred_entities') or [])) or '<li>Unknown</li>'}
  </ul>
  <h3>Observed from frontend (used to shape synthetic backend)</h3>
  <p><strong>API paths:</strong> {', '.join((report.get('backend_inference') or {}).get('observed_from_frontend', {}).get('api_paths') or []) or 'None'}</p>
  <p><strong>Form fields:</strong> {', '.join((report.get('backend_inference') or {}).get('observed_from_frontend', {}).get('form_fields') or []) or 'None'}</p>
  <p><strong>UI entity words:</strong> {', '.join((report.get('backend_inference') or {}).get('observed_from_frontend', {}).get('string_entities') or []) or 'None'}</p>
  <h3>Inferred business logic</h3>
  <ul>
    {''.join(f"<li>{x}</li>" for x in ((report.get('backend_inference') or {}).get('inferred_business_logic') or [])) or '<li>Unknown</li>'}
  </ul>
